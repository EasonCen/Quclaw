...
class PromptBuilder:
    ...
    def _load_bootstrap_context(self) -> str:
        ...
        if bootstrap_path.exists():
            parts.append(self._load_workspace_prompt_file(bootstrap_path))

        agents_path = self.context.config.workspace / "AGENTS.md"
        if agents_path.exists():
            parts.append(self._load_workspace_prompt_file(agents_path))
    ...
    def _load_workspace_prompt_file(self, path: Path) -> str:
        """Load a workspace prompt file and render config-backed placeholders."""
        return self._render_template(path.read_text(encoding="utf-8").strip())

    def _render_template(self, content: str) -> str:
        """Render simple workspace placeholders inside prompt context files."""
        rendered = content
        for key, value in self.context.config.template_vars().items():
    ...
    def _build_channel_hint(self, source: "EventSource") -> str:
        """Build platform hint."""
        if source.is_heartbeat:
            return "You are running as a silent heartbeat check. Your response will be consumed internally and not sent to the user."
        if source.is_cron:
